...
var (
	ErrEvidence    = failure.MustDefine(failure.Definition{Code: "fathomry.operation.evidence_capacity", Component: "operation", Version: 1})
	ErrPending     = failure.MustDefine(failure.Definition{Code: "fathomry.operation.evidence_pending", Component: "operation", Version: 1})
	ErrObservation = failure.MustDefine(failure.Definition{Code: "fathomry.operation.observation_failed", Component: "operation", Version: 1})
)
...
func NewInbox[T any](capacity int, bytes int64) (*Inbox[T], error) {
	if capacity <= 0 || bytes <= 0 {
		return nil, ErrEvidence.New(failure.Attribution{})
	}
...
func (inbox *Inbox[T]) reserve(bytes int64) error {
	if inbox == nil || inbox.queue == nil || bytes <= 0 {
		return ErrEvidence.New(failure.Attribution{})
	}
	inbox.mu.Lock()
...
func (inbox *Inbox[T]) Next(ctx context.Context) (*DeliveryRecord[T], error) {
	...
	if err := ctx.Err(); err != nil {
		return nil, ErrWait.New(failure.Attribution{}, err, context.Cause(ctx))
	}
	select {
	case delivery := <-inbox.queue:
		return delivery, nil
	case <-ctx.Done():
		return nil, ErrWait.New(failure.Attribution{}, ctx.Err(), context.Cause(ctx))
	}
}
...
// the same read-only outcome offered to the public caller. Do not copy this owner;
...
func (delivery *DeliveryRecord[T]) Release() error {
	...
	default:
		return ErrPending.New(delivery.receipt.state.attribution)
	}
...
func NewObserver(capacity int) (*Observer, error) {
	if capacity <= 0 {
		return nil, ErrObservation.New(failure.Attribution{})
	}
...
func (observer *Observer) ExportOne(ctx context.Context, export func(context.Context, Event) error) (err error) {
	if observer == nil || observer.queue == nil || ctx == nil || export == nil {
		return ErrObservation.New(failure.Attribution{})
	}
	if ctx.Err() != nil {
		return ErrObservation.New(failure.Attribution{}, ctx.Err(), context.Cause(ctx))
	}
	var event Event
	select {
	...
	defer func() {
		_ = recover()
		if !returned {
			err = ErrObservation.New(failure.Attribution{})
		}
	}()
	exportErr := export(context.WithValue(ctx, observationKey{}, true), event)
	returned = true
	if exportErr != nil {
		return ErrObservation.New(failure.Attribution{}, exportErr)
	}
	return nil
}
