...
var (
	ErrAssembly       = failure.MustDefine(failure.Definition{Code: "fathomry.source.assembly_failed", Component: "source", Version: 1})
	ErrSelection      = failure.MustDefine(failure.Definition{Code: "fathomry.source.invalid_selection", Component: "source", Version: 1})
	ErrInitialization = failure.MustDefine(failure.Definition{Code: "fathomry.source.initialization_failed", Component: "source", Version: 1})
	ErrCleanup        = failure.MustDefine(failure.Definition{Code: "fathomry.source.cleanup_failed", Component: "source", Version: 1})
	ErrIncomplete     = failure.MustDefine(failure.Definition{Code: "fathomry.source.cleanup_incomplete", Component: "source", Version: 1})
)
...
func sharedSelection[C any](name string, from *Assembly, selected Selection[C], mode Ownership) Selection[C] {
	...
	if selected.spec != nil {
		spec.description = copyDescription(selected.spec.description)
	}
	return Selection[C]{spec: spec}
}
...
func Assemble(ctx, cleanupCtx context.Context, scope string, selected ...Spec) (*Assembly, error) {
	...
	fail := func(err error) (*Assembly, error) {
		...
		return assembly, ErrAssembly.New(failure.Attribution{Operation: "assemble", Assembly: scope}, err, cleanup)
	}
	for _, spec := range specs {
		...
		if !acquired || value.release == nil || nilValue(value.capability) {
			return fail(ErrInitialization.New(attribution(scope, spec.description, "construct"),
...
func attribution(scope string, description Description, operation string) failure.Attribution {
	return failure.Attribution{Assembly: scope, Source: description.Identity.Name,
...
func existing(spec *specification, receiver *Assembly, acquire bool) (*record, error) {
	fail := func() (*record, error) { return nil, ErrSelection.New(failure.Attribution{Operation: "share"}) }
...
func Bind[C any](assembly *Assembly, selected Selection[C]) (C, Info, error) {
	...
	for _, entry := range assembly.entries {
		...
		return value, Info{Scope: resource.info.Scope, Configuration: copyDescription(resource.info.Configuration)}, nil
	}
	return fail()
}
...
func (assembly *Assembly) Close(ctx context.Context) error {
	if assembly == nil || assembly.gate == nil || ctx == nil {
		return ErrSelection.New(failure.Attribution{Operation: "close"})
	}
	assembly.mu.Lock()
	...
	for index := len(assembly.entries) - 1; index >= 0; index-- {
		...
		if result.Err != nil {
			resource.cleanupErrors = append(resource.cleanupErrors,
				ErrCleanup.New(attribution(resource.info.Scope, resource.info.Configuration, "release"), result.Err))
		}
...
func (assembly *Assembly) closeError(interrupted error) error {
	...
	location := failure.Attribution{Operation: "close", Assembly: assembly.scope}
	if pending {
