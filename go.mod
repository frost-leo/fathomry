...
require (
	...
	// routing, callbacks, replay and encoded/decoded integrity before upgrades.
	// Retire the local correction only after the original counterexamples pass
	// against an official release. Keep source and local revision provenance.
...
// TODO(gh-53): Retire only after upstream stream-scoped QPACK cancellation,
// bounded dynamic decoding, control-stream failure, early-response upload join
// and qlog error/retry/release gates pass together in the consuming graph.
replace github.com/nukilabs/quic-go => ./third_party/nuki-quic-go

replace github.com/nukilabs/qpack => ./third_party/nuki-qpack
