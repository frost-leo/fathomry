...
// Assess binds actual immutable source metadata/limits to the supplied effective
// profile and consuming build. The caller owns profile correctness, requirement
...
func Assess(build Build, access *source.Access, profile Profile, requirements []Requirement, records []Record) (Report, error) {
	...
	if info.Configuration.Revision == "" || len(requirements) == 0 || len(requirements) > 32 || len(records) > 128 || !validCombination(actual) {
		return Report{}, ErrInvalid.New(failure.Attribution{})
	}
...
var (
	ErrUnsupported = failure.MustDefine(failure.Definition{Code: "fathomry.compatibility.unsupported", Component: "compatibility", Version: 1})
	ErrUnverified  = failure.MustDefine(failure.Definition{Code: "fathomry.compatibility.unverified", Component: "compatibility", Version: 1})
)
...
func (report Report) Require(policy Policy) error {
	if len(report.Decisions) == 0 {
		return ErrUnverified.New(failure.Attribution{})
	}
	unverified := false
	for _, decision := range report.Decisions {
		switch decision.Status {
		case Incompatible:
			return ErrUnsupported.New(failure.Attribution{})
		case Tested:
		case Unknown:
