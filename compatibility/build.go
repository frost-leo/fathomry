...
var ErrInvalid = failure.MustDefine(failure.Definition{Code: "fathomry.compatibility.invalid", Component: "compatibility", Version: 1})
...
func FromBuildInfo(info *debug.BuildInfo, request BuildRequest) (Build, error) {
	if len(request.SDKModules) > 64 || len(request.DisclosePaths) > 64 {
		return Build{}, ErrInvalid.New(failure.Attribution{})
	}
