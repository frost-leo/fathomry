...
func sourceAccess(t testing.TB) (*source.Assembly, source.Selection[struct{}], *source.Access) {
	t.Helper()
	prepared, err := source.Prepare(source.Schema[struct{}]{Format: 7},
		source.Input{Identity: source.Identity{Provider: "fixture.local", Name: "output"}, Format: 7})
	if err != nil {
		t.Fatal(err)
	}
	selection := source.WithLimits(source.Select(prepared, func(context.Context, struct{}) (source.Resource[struct{}], error) {
	...
	assembly, err := source.Assemble(context.Background(), context.Background(), "owner", selection)
	if err != nil {
		t.Fatal(err)
	}
	access, err := source.AccessFor(assembly, selection)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
...
func assess(t testing.TB, build compatibility.Build, access *source.Access, mode compatibility.Profile, records ...compatibility.Record) compatibility.Report {
	t.Helper()
...
func TestProfilesBuildsAndRecordsAreBoundedAndCopied(t *testing.T) {
	...
	borrower, err := source.Assemble(context.Background(), context.Background(), "borrower", borrowed)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	alias, err := source.AccessFor(borrower, borrowed)
	if err != nil {
		t.Fatal(err)
	}
