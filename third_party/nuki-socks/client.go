...
func (d *Dialer) connect(ctx context.Context, c net.Conn, address string, cmd Command) (_ *Addr, ctxErr error) {
	host, port, err := splitHostPort(address)
	if err != nil {
		return nil, err
	}
	if deadline, ok := d.deadline(ctx); ok && !deadline.IsZero() {
	...
	b = b[:0]
	b = append(b, Version5, byte(cmd), 0)
