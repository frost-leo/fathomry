...
func decodeTrailers(r io.Reader, hf *headersFrame, maxHeaderBytes int, decoder *qpack.Decoder, qlogger qlogwriter.Recorder, streamID quic.StreamID) (http.Header, error) {
	return decodeTrailersContext(context.Background(), r, hf, maxHeaderBytes, decoder, qlogger, streamID)
}

func decodeTrailersContext(ctx context.Context, r io.Reader, hf *headersFrame, maxHeaderBytes int, decoder *qpack.Decoder, qlogger qlogwriter.Recorder, streamID quic.StreamID) (http.Header, error) {
	...
	decodeFn := decoder.DecodeForStreamContext(ctx, int64(streamID), b, uint64(maxHeaderBytes))
