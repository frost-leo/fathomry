...
func (c *ClientConn) roundTrip(req *http.Request) (*http.Response, error) {
	...
	var ownedBody *ownedUploadBody
	if req.Body != nil {
		req = req.Clone(req.Context())
		ownedBody = &ownedUploadBody{ReadCloser: req.Body}
		req.Body = ownedBody
	}
	...
	if err != nil {
		if ownedBody != nil {
			err = errors.Join(err, ownedBody.Close())
		}
	...
	rsp.Body = &joinedResponseBody{ReadCloser: rsp.Body, done: done, upload: str.upload, input: ownedBody}
