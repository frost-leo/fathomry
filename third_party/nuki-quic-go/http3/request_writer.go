...
func (w *requestWriter) encodeHeaders(req *http.Request, addGzipHeader bool, trailers string, contentLength int64, doQlog bool) ([]qlog.HeaderField, error) {
	...
	// and accept-encoding for gzip, before sorting, so that they are emitted in
	// their requested position in the header order.
	...
	// Sort the headers, honoring req.Header[HeaderOrderKey] if present.
