...
type Transport struct {
	// FathomryManaged retains peer stream-end evidence for bodyless responses.
	FathomryManaged bool
...
type ClientConn struct {
	fathomryRequests sync.WaitGroup
	t                *Transport
...
func (cc *ClientConn) roundTrip(req *http.Request, streamf func(*clientStream)) (*http.Response, error) {
	...
	cc.fathomryRequests.Add(1)
	go func() { defer cc.fathomryRequests.Done(); cs.doRequest(req, streamf) }()
...
func encodeRequestHeaders(req *http.Request, addGzipHeader bool, peerMaxHeaderListSize uint64, pseudoHeaderOrder []string, headerOrder []string, userAgent string, disableCookieSplit bool, headerf func(name, value string)) (httpcommon.EncodeHeadersResult, error) {
	if req.Context().Value(fathomryExactHeadersKey{}) == true {
...
func (rl *clientConnReadLoop) handleResponse(cs *clientStream, f *MetaHeadersFrame) (*http.Response, error) {
	...
	if cs.isHead {
		if cs.cc.t.FathomryManaged {
			res.Body = fathomryEmptyBody{cs}
		} else {
			res.Body = noBody
		}
		return res, nil
