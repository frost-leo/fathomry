...
func buildHTTP3Transport(cfg *http3Config) (http.RoundTripper, error) {
	...
	if cfg.proxyURL != "" {
		...
		t3.Dial = newSOCKS5QUICDialer(cfg.proxyURL, cfg.compat)
	}
...
func (rt *roundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	work, done, err := rt.compat.begin(req.Context())
	if err != nil {
		return nil, err
	}
	...
	response, err := rt.roundTrip(req.WithContext(work))
...
func (rt *roundTripper) dialTLSWithSetup(ctx context.Context, network, addr string, setup bool) (net.Conn, error) {
	...
	if !setup {
		...
		if ok && cachedKind == negotiatedKind {
			return conn, nil
		}
