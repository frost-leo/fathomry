...
func socks5UDPAssociate(ctx context.Context, proxyAddr string, auth *socks5Auth, clientUDPAddr *net.UDPAddr) (controlConn net.Conn, relayAddr *net.UDPAddr, err error) {
	...
	if state, ok := ctx.Value(compatibilityContextKey{}).(*compatibilityState); ok {
		tcpConn, err = state.dial(ctx, "tcp", proxyAddr, d.DialContext)
	} else {
...
func newSOCKS5QUICDialer(proxyURL string, states ...*compatibilityState) func(ctx context.Context, addr string, tlsCfg *tls.Config, cfg *quic.Config) (*quic.Conn, error) {
	return func(ctx context.Context, addr string, tlsCfg *tls.Config, cfg *quic.Config) (*quic.Conn, error) {
		...
		if len(states) != 0 && states[0] != nil {
			state = states[0]
			work, finish, err := state.begin(ctx)
			if err != nil {
				return nil, err
			}
			ctx = context.WithValue(work, compatibilityContextKey{}, state)
			done = finish
			defer func() {
				if !handed {
					done()
		...
		go func() {
			...
			transportErr := quicTransport.Close()
			packetErr := packetConn.Close()
			if state != nil {
				state.failed(errors.Join(transportErr, packetErr))
			}
		}()
