...
func (c *connectDialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	case "http":
		rawConn, err = c.dialRaw(ctx, network, c.ProxyUrl.Host)
	...
	case "https":
		if c.DialTLS != nil {
			...
			connection, err := c.dialRaw(ctx, network, c.ProxyUrl.Host)
			if err != nil {
				return nil, err
			}
			tlsConn := tls.Client(connection, &tlsConf)
...
func (c *connectDialer) dialRaw(ctx context.Context, network, address string) (net.Conn, error) {
	if c.compat != nil {
		return c.compat.dial(ctx, network, address, c.Dialer.DialContext)
	}
