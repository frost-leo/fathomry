...
- Fathomry local compatibility extensions: explicit owner-scoped native lazy error
  and Activity/Workflow/Nexus description decoding, preserving native concrete
  types and Failure data; and a construction-only eager dispatcher bridge for
  managed Workers using a separately owned polling Client. These are local
  integration contracts, not upstream APIs or changes to execution protocols.

- Fathomry local compatibility extension: opt-in scoped Workflow Update handles
  protect cached decoding while preserving native Nexus completion classification
  through `client.FathomryScopeWorkflowUpdateHandleV1`. This is not an upstream API.

- Fathomry local compatibility extension: opt-in worker ownership tracking and
  cancellable observation of complete local shutdown through
  `worker.FathomryWaitStoppedV1`. Native Stop timeouts, Workflow commands and
  Activity retry outcomes are unchanged. This is not an upstream release.
...
- Local patch: roll back a freshly allocated Client connection if construction
  exits before ownership transfers. Existing shared connections are not closed
  by this rollback.
