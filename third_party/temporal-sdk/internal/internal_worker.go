...
func newWorkflowTaskWorkerInternal(
	...
	bwo := baseWorkerOptions{
		fathomryLifetime:             params.fathomryLifetime,
	...
	localActivityWorker := newBaseWorker(baseWorkerOptions{
		fathomryLifetime: laParams.fathomryLifetime,
...
func (r *registry) RegisterWorkflowWithOptions(
	...
	if ok {
		if r.fathomryLifetime != nil {
			panic("temporal: managed lifecycle requires SDK workflow definitions, not internalbindings factories")
		}
...
func (aw *AggregatedWorker) start() error {
	...
	if !util.IsInterfaceNil(aw.workflowWorker) {
		...
		if aw.eagerDispatcher != nil {
			aw.eagerDispatcher.registerWorker(aw.workflowWorker)
		}
	}
	if !util.IsInterfaceNil(aw.activityWorker) {
		...
		if err := aw.activityWorker.Start(); err != nil {
			...
			if !util.IsInterfaceNil(aw.workflowWorker) {
				if aw.workflowWorker.worker.isWorkerStarted {
					if aw.eagerDispatcher != nil {
						aw.eagerDispatcher.deregisterWorker(aw.workflowWorker)
					}
...
func NewAggregatedWorker(client *WorkflowClient, taskQueue string, options WorkerOptions) *AggregatedWorker {
	...
	workerParams := workerExecutionParameters{
		...
		cache:                            cache,
		fathomryLifetime:                 cache.fathomryLifetime,
	...
	registry := newRegistryWithOptions(registryOptions{disableAliasing: options.DisableRegistrationAliasing})
	registry.fathomryLifetime = cache.fathomryLifetime
	...
	aw = &AggregatedWorker{
		client:                       client,
		eagerDispatcher:              client.eagerDispatcher,
		workflowWorker:               workflowWorker,
		activityWorker:               activityWorker,
		sessionWorker:                sessionWorker,
