...
func (c *lru) Delete(key string) {
	c.deleteIf(key, nil, false)
}

func (c *lru) DeleteIf(key string, expected any) {
	c.deleteIf(key, expected, true)
}

func (c *lru) deleteIf(key string, expected any, conditional bool) {
	c.mut.Lock()
	defer c.mut.Unlock()

	elt := c.byKey[key]
	if elt != nil && (!conditional || elt.Value.(*cacheEntry).value == expected) {
