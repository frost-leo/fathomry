...
func newBaseWorker(
	...
	tss := newTrackingSlotSupplier(options.slotSupplier, trackingSlotSupplierOptions{
		fathomryLifetime: options.fathomryLifetime,
		logger:           logger,
...
func (bw *baseWorker) tryReserveSlot() *SlotPermit {
	if !bw.options.fathomryLifetime.enterExternal() {
		return nil
	}
	defer bw.options.fathomryLifetime.end()
	if bw.isStop() {
		return nil
	}
...
func (bw *baseWorker) pushEagerTask(task eagerTask) {
	if bw.options.fathomryLifetime != nil {
		bw.fathomryEagerMu.Lock()
		stopped := bw.isStop()
		if !stopped {
			bw.eagerTaskQueueCh <- task
		}
		bw.fathomryEagerMu.Unlock()
		if stopped {
			bw.releaseSlot(task.permit, SlotReleaseReasonUnused)
		}
		return
	}
