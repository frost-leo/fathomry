...
func (w *workflowExecutionContextImpl) retryLocalActivity(lar *localActivityResult) bool {
	...
	if retryBackoff > 0 && retryBackoff <= w.workflowInfo.WorkflowTaskTimeout {
		...
		w.wth.registry.fathomryLifetime.add()
		time.AfterFunc(retryBackoff, func() {
			defer w.wth.registry.fathomryLifetime.end()
			// Send retry signal
			select {
...
func (i *temporalInvoker) Heartbeat(ctx context.Context, details *commonpb.Payloads, skipBatching bool) error {
	...
	if (err == nil || isActivityCanceled) && !skipBatching {
		...
		i.fathomryLifetime.add()
		go func() {
			defer i.fathomryLifetime.end()
			select {
...
func (ath *activityTaskHandlerImpl) Execute(taskQueue string, t *workflowservice.PollActivityTaskQueueResponse) (result activityTaskResult, err error) {
	...
	invoker.(*temporalInvoker).fathomryLifetime = ath.registry.fathomryLifetime
