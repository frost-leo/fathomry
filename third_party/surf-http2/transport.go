...
func (t *Transport) maxHeaderListSize() uint32 {
	return t.maxHeaderListSizeFor(t.MaxHeaderListSize)
}

func (t *Transport) maxHeaderListSizeFor(configured uint32) uint32 {
	n := int64(configured)
...
func (t *Transport) newClientConn(c net.Conn, singleUse bool, internalStateHook func()) (*ClientConn, error) {
	...
	for _, setting := range t.Settings {
		...
		case SettingHeaderTableSize:
			maxHeaderTableSize = setting.Val
		}
	}

	cc.fr.ReadMetaHeaders = hpack.NewDecoder(maxHeaderTableSize, nil)
	cc.fr.MaxHeaderListSize = t.maxHeaderListSizeFor(maxHeaderListSize)
	...
	if cc.t.ConnectionFlow != 0 {
		conf.MaxUploadBufferPerConnection = int32(cc.t.ConnectionFlow)
	}
...
func (cc *ClientConn) roundTrip(req *http.Request, streamf func(*clientStream)) (*http.Response, error) {
	...
	go func() { defer finish(); cs.doRequest(req, streamf) }()
