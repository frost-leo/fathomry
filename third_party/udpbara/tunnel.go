...
type Tunnel struct {
	connecting map[net.Conn]struct{}
...
func (t *Tunnel) ConnectContext(ctx context.Context) error {
	...
	go func() { defer t.workers.Done(); t.readLoop() }()
	go func() { defer t.workers.Done(); t.monitorControl() }()
...
func (t *Tunnel) doConnect(ctx context.Context) (ctrl net.Conn, relayAddr *net.UDPAddr, remoteUDP *net.UDPConn, err error) {
	...
	stop, trackErr := t.fathomryConnecting(ctx, ctrl)
	if trackErr != nil {
		return nil, nil, nil, trackErr
	}
	defer stop()
	defer func() { err = fathomryContextError(ctx, err) }()
...
func (t *Tunnel) DialContext(ctx context.Context, target string) (*Connection, error) {
	...
	if t.closing.Load() || ctx.Err() != nil {
		t.mu.Unlock()
		return nil, fathomryContextError(ctx, fmt.Errorf("tunnel dial stopped"))
	}
	t.connects.Add(1)
	t.mu.Unlock()
	defer t.connects.Done()
