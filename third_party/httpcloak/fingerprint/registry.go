...
func Register(name string, preset *Preset) {
	...
	customPresets.Store(name, clonePreset(preset))
}
...
func RegisterStrict(name string, preset *Preset) error {
	...
	if _, loaded := customPresets.LoadOrStore(name, clonePreset(preset)); loaded {
		return fmt.Errorf("preset name %q already registered", name)
	}
	return nil
}
