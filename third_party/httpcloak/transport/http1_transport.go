	...
	if err == nil && conn != nil {
		...
		if req.Body != nil && req.Body != http.NoBody {
			...
			if gerr != nil {
				...
				return nil, WrapError("request", host, port, "h1", errors.Join(err, gerr, cleanup))
			}
			if rc == nil {
				return nil, errors.Join(err, errors.New("httpcloak: replay factory returned no body"))
			}
			req.Body = rc
		}
	}
...
func (t *HTTP1Transport) createConn(ctx context.Context, host, port, scheme string) (*http1Conn, error) {
	...
	if t.config != nil && t.config.FathomryDialTCP != nil {
		rawConn, err = t.config.FathomryDialTCP(ctx, connectHost, port)
		if err != nil {
			return nil, err
		}
	} else if t.proxy != nil && t.proxy.URL != "" {
	...
	reader := io.Reader(conn.conn)
	if t.config != nil && t.config.FathomryMaxHeaderBytes > 0 {
		conn.headerLimit = &headerLimitReader{reader: bufio.NewReaderSize(conn.conn, 64*1024), finished: true}
		reader = conn.headerLimit
	}
	conn.br = bufio.NewReaderSize(reader, 64*1024)     // 64KB read buffer
...
func (t *HTTP1Transport) doRequest(conn *http1Conn, req *http.Request) (*http.Response, error) {
	...
	// Write request
	writeErr := t.writeRequest(conn, req)
	if t.config != nil && t.config.FathomryDialTCP != nil {
	...
	if conn.headerLimit != nil {
		conn.headerLimit.remaining = t.config.FathomryMaxHeaderBytes - int64(conn.br.Buffered())
		if conn.headerLimit.remaining <= 0 {
			return nil, FathomryErrHeaderLimit
		}
		conn.headerLimit.ending = 0
		conn.headerLimit.finished = false
	}
	for count := 0; count < 16; count++ {
		...
		if trace := httptrace.ContextClientTrace(req.Context()); trace != nil && trace.Got1xxResponse != nil {
			header, _, _ := FathomryHeaderMetadata(resp.Header)
			if err := trace.Got1xxResponse(resp.StatusCode, textproto.MIMEHeader(header)); err != nil {
				return nil, err
			}
		}
		if conn.headerLimit != nil {
...
func (t *HTTP1Transport) writeRequest(conn *http1Conn, req *http.Request) (resultErr error) {
	// Request line
	...
	// speaks 1.1.
	connName := "Connection"
	if key, ok := resolveHeaderKey(req.Header, "Connection"); ok {
		connName = key
	}
	connValue := headerVal(req.Header, "Connection")
	if req.Close {
		connValue = "close"
	}
	...
	useChunked := req.Body != nil && req.Body != http.NoBody && req.ContentLength <= 0 && headerVal(req.Header, "Content-Length") == ""
	...
	if req.Body != nil {
		defer func() { resultErr = errors.Join(resultErr, req.Body.Close()) }()
		if useChunked {
			...
			written, err := io.Copy(conn.bw, io.LimitReader(req.Body, req.ContentLength))
			if written != req.ContentLength {
				return errors.Join(FathomryErrRequestLength, io.ErrUnexpectedEOF, err)
			}
			if err != nil {
				return err
			}
			extra, err := io.ReadAll(io.LimitReader(req.Body, 1))
			if len(extra) != 0 {
				return errors.Join(FathomryErrRequestLength, err)
			}
			if err != nil {
				return err
			}
...
func (t *HTTP1Transport) writeHeadersInOrder(w *bufio.Writer, req *http.Request, useChunked bool) {
	...
	if !written["Content-Length"] && !useChunked {
		if mapKey, ok := resolveHeaderKey(req.Header, "Content-Length"); ok {
			for _, v := range req.Header[mapKey] {
				fmt.Fprintf(w, "%s: %s\r\n", mapKey, v)
			}
		} else if req.ContentLength > 0 {
...
func (t *HTTP1Transport) shouldKeepAlive(req *http.Request, resp *http.Response) bool {
	...
	if strings.EqualFold(headerVal(req.Header, "Connection"), "close") {
		return false
	}
...
func (t *HTTP1Transport) putIdleConn(key string, conn *http1Conn) {
	...
	if len(conns) >= t.maxIdleConnsPerHost {
		...
		oldConn := conns[0]
		conns = conns[1:]
		t.fathomryCleanup(oldConn.close)
	}
