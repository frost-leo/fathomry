...
func (g *connBodyGuard) Close() error {
	...
	if g.conn.managed {
		g.conn.h2Conn.FathomryWaitRequests()
	}
	g.release()
	return err
}
...
func (t *HTTP2Transport) getOrCreateConn(ctx context.Context, host, port, key string) (*persistentConn, error) {
	...
	if exists {
		t.fathomryCleanup(func() { t.retire(conn) })
		delete(t.conns, key)
	}
	t.connsMu.Unlock()
	...
	v, err, _ := t.dialGroup.Do(key, func() (interface{}, error) {
		...
		if t.closed {
			t.connsMu.Unlock()
			t.fathomryCleanup(newConn.close)
			return nil, fmt.Errorf("http2: transport closed")
		}
...
func (t *HTTP2Transport) createConn(ctx context.Context, host, port string) (*persistentConn, error) {
	...
	if !t.echConfigured() || t.echFailures.disabled(t.config, host) {
		return t.establishConn(ctx, host, port, t.echConfigured() && t.echFailures.disabled(t.config, host))
	}
	...
	if echCausedDialFailure(err) || errors.Is(err, context.DeadlineExceeded) {
		t.echFailures.mark(t.config, host)
...
alpnCheck:
	...
	if t.config != nil && t.config.FathomryMaxHeaderBytes > 0 {
		h2Transport.MaxHeaderListSize = uint32(t.config.FathomryMaxHeaderBytes)
		h2Transport.FathomryManaged = true
	}
	...
	return &persistentConn{
		managed:        t.config != nil && t.config.FathomryDialTCP != nil,
		host:           host,
		tlsConn:        tlsConn,
		h2Conn:         h2Conn,
		createdAt:      time.Now(),
...
func (t *HTTP2Transport) Connect(ctx context.Context, host, port string) error {
	...
	if oldConn, exists := t.conns[key]; exists {
		...
		if !t.isConnUsable(oldConn) {
			t.fathomryCleanup(func() { t.retire(oldConn) })
		} else {
