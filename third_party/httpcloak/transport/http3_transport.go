...
func (t *HTTP3Transport) Close() error {
	t.closeOnce.Do(func() { t.closeErr = t.fathomryClose() })
	return t.closeErr
}

func (t *HTTP3Transport) fathomryClose() error {
	// Capture transport pointer under lock to avoid data race with recreateTransport/Refresh
	t.mu.RLock()
	transport := t.transport
	t.mu.RUnlock()
	...
	var causes []error
	if transport != nil {
		causes = append(causes, transport.Close())
	}
	...
	if t.masqueConn != nil {
		causes = append(causes, t.masqueConn.Close())
	}
...
func (t *HTTP3Transport) Connect(ctx context.Context, host, port string) error {
	...
	var echConfigList []byte
	if !t.disableECH && !t.echFailures.disabled(t.config, host) {
