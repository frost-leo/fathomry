...
func (b *Body) read() (result g.Result[g.Bytes]) {
	if b.Reader == nil {
	...
	defer func() {
		if err := b.Close(); err != nil {
			result = g.Err[g.Bytes](errors.Join(result.Err(), err))
		}
	}()
	...
	_, err := io.Copy(buf, io.LimitReader(b.Reader, readLimit))
	if err != nil {
		return g.Err[g.Bytes](err)
	}
	if int64(buf.Len()) > limit {
		return g.Err[g.Bytes](ErrFathomryBodyLimit)
	}
	if b.contentLength >= 0 && int64(buf.Len()) != b.contentLength {
		return g.Err[g.Bytes](io.ErrUnexpectedEOF)
	}
...
// It does not drain an unread stream: draining can wait indefinitely for more data.
