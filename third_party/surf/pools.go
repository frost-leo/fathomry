...
func (reader *decodedReadCloser) Read(buffer []byte) (int, error) {
	...
	reader.reads.Add(1)
	reader.mu.Unlock()
	defer reader.reads.Done()
	count, err := reader.decoder.Read(buffer)
	if err == io.EOF {
		if completeErr := fathomrySourceEOF(reader.source); completeErr != nil {
			err = completeErr
		}
	}
	return count, err
...
func (reader *decodedReadCloser) Close() error {
	reader.once.Do(func() {
		...
		reader.closed = true
		reader.mu.Unlock()
		sourceErr := reader.source.Close()
		reader.reads.Wait()
		reader.closeErr = errors.Join(sourceErr, reader.decoder.Close())
	})
