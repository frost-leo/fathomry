...
func (ut *uquicTransport) initTransport() error {
	...
	if maximum := ut.client.fathomry.control.MaxHeaderBytes; maximum > 0 {
		ut.http3tr.MaxResponseHeaderBytes = int(maximum)
	}
...
func (ut *uquicTransport) createUDPPacketConn() (net.PacketConn, error) {
	ctx := ut.client.fathomry.control.ConstructionContext
	if ctx == nil {
		ctx = context.Background()
	}
	if conn, err := ut.client.fathomryListen(ctx, "udp4", ":0"); err == nil {
		return conn, nil
	}

	return ut.client.fathomryListen(ctx, "udp6", ":0")
}
...
func (ut *uquicTransport) dial(
	...
	ctx, finishContext := ut.client.fathomry.bindContext(ctx)
	defer finishContext()
	end, err := ut.client.fathomry.enter(ctx)
	if err != nil {
		return nil, err
	}
	defer end()
...
func (ut *uquicTransport) dialSOCKS5(
	...
	association := &fathomryAssociation{}
	var stopConnecting []func()
	dialer.ProxyDial = func(ctx context.Context, network, address string) (net.Conn, error) {
		conn, err := ut.client.fathomryDial(ctx, network, address)
		if conn != nil {
			association.closers = append(association.closers, conn)
			done := make(chan struct{})
			stop := context.AfterFunc(ctx, func() { _ = conn.Close(); close(done) })
			stopConnecting = append(stopConnecting, func() {
				if !stop() {
					<-done
				}
			})
	...
	dialer.ProxyPacketDial = func(ctx context.Context, network, address string) (net.PacketConn, error) {
		conn, err := ut.client.fathomryListen(ctx, network, address)
		if conn != nil {
			association.closers = append(association.closers, conn)
		}
		return conn, err
	...
	for _, stop := range stopConnecting {
		stop()
	}
	if err != nil {
	...
	nativeUDP, ok := conn.(*socks5.UDPConn)
	if !ok {
		return nil, errors.New("surf: unsupported SOCKS UDP connection")
	}
	packetConn := quicconn.New(&fathomrySOCKSConn{UDPConn: nativeUDP}, proxyUDP, quicconn.EncapRaw)
	transport := &quic.Transport{Conn: packetConn}
	association.closers = append([]io.Closer{transport, packetConn}, association.closers...)
	quicConn, err := transport.Dial(ctx, targetUDP, tlsCfg, cfg)
	if err != nil {
		packetConn.Close()
		return nil, fmt.Errorf("QUIC dial: %w", err)
	}
	if err := ut.client.fathomry.own(association); err != nil {
		_ = quicConn.CloseWithError(0, "")
		return nil, err
	}
	finished, err := ut.client.fathomry.enter(ut.client.fathomry.ctx)
	if err != nil {
		_ = quicConn.CloseWithError(0, "")
		return nil, err
	}
	go func() {
		...
		ut.client.fathomry.remember(association.Close())
		ut.client.fathomry.disown(association)
	}()
...
func (ut *uquicTransport) handleError(req *http.Request, err error) (*http.Response, error) {
	...
	if req.Body != nil && req.Body != http.NoBody {
		...
		if bodyErr != nil {
			return nil, errors.Join(err, fmt.Errorf("failed to restore body: %w", bodyErr))
		}
...
func (ut *uquicTransport) Close() error {
	ut.closeOnce.Do(func() { ut.closeErr = ut.closeOwned() })
	return ut.closeErr
}
...
func (ut *uquicTransport) closeOwned() error {
	...
	if ut.quictr != nil {
		cleanup = append(cleanup, ut.quictr.Close())
	}

	if ut.pconn != nil {
		cleanup = append(cleanup, ut.pconn.Close())
	}

	if ut.fallbackTransport != nil {
		if c, ok := ut.fallbackTransport.(interface{ Close() error }); ok {
			cleanup = append(cleanup, c.Close())
		} else if c, ok := ut.fallbackTransport.(interface{ CloseIdleConnections() }); ok {
			c.CloseIdleConnections()
		}
	}
