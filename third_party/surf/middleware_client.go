...
func defaultTransportMW(client *Client) error {
	transport := &http.Transport{
		DialContext:           client.fathomryDial,
	...
	transport.DialTLSContext = func(ctx context.Context, network, address string) (net.Conn, error) {
		return client.fathomryTLS(ctx, network, address, transport.DialContext)
	}
...
func proxyMW(client *Client, proxy g.String) error {
	...
	if proxy.IsEmpty() {
		transport.Proxy = nil
		transport.DialContext = client.fathomryDial
		return nil
	}
	...
	dialer.DialSocket = client.fathomryDial
	dialer.ConnectionContext = http2.FathomryWithWork(client.fathomry.ctx, client.fathomry.enter)
	dialer.DisableReuse = true
	dialer.MaxHeaderBytes = client.fathomry.control.MaxProxyHeaderBytes
	dialer.DialTLSContext = func(ctx context.Context, network, address string) (net.Conn, string, error) {
		...
		defer end()
		raw, err := client.fathomryDial(ctx, network, address)
		if err != nil {
			return nil, "", err
		}
		config := &tls.Config{NextProtos: []string{"h2", "http/1.1"}}
		if configured := client.fathomry.control.ProxyTLSConfig; configured != nil {
			config = configured.Clone()
		}
		if config.ServerName == "" {
			config.ServerName = dialer.ProxyURL.Hostname()
		}
	...
	if err := client.fathomry.own(dialer); err != nil {
		return err
	}
