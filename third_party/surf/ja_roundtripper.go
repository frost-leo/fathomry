...
func newRoundTripper(ja *JA, base http.RoundTripper) http.RoundTripper {
	...
	if ja.builder.cli.tlsConfig.ClientSessionCache != nil && !ja.builder.cli.tlsConfig.SessionTicketsDisabled {
...
func (rt *roundtripper) handleHTTPSRequest(req *http.Request) (*http.Response, error) {
	...
	h2Err := err
	if rt.ja.builder.forceHTTP2 {
		return nil, h2Err
	}

	// HTTP/2 failed - fallback to HTTP/1.1
	if err := req.Context().Err(); err != nil {
		return nil, errors.Join(h2Err, err, context.Cause(req.Context()))
	}
	...
	if req.Body != nil && req.Body != http.NoBody {
		...
		if bodyErr != nil {
			return nil, errors.Join(h2Err, fmt.Errorf("surf: failed to restore body for fallback: %w", bodyErr))
		}
		req.Body = body
	}
...
func (rt *roundtripper) buildHTTP2Transport() *http2.Transport {
	...
	if maximum := rt.ja.builder.cli.fathomry.control.MaxHeaderBytes; maximum > 0 {
		t.MaxHeaderListSize = uint32(maximum)
	}
...
func (rt *roundtripper) tlsHandshake(ctx context.Context, network, addr string, forceHTTP1 bool) (result *utls.UConn, err error) {
	ctx, finishContext := rt.ja.builder.cli.fathomry.bindContext(ctx)
	defer finishContext()
	end, err := rt.ja.builder.cli.fathomry.enter(ctx)
	if err != nil {
		return nil, err
	}
	defer end()
	...
	defer func() {
		if result == nil {
			...
			err = fathomryFailure(err, rawConn.Close())
		}
	}()
	...
	config := &utls.Config{
		ServerName:             host,
		RootCAs:                rt.ja.builder.cli.tlsConfig.RootCAs,
		VerifyPeerCertificate:  rt.ja.builder.cli.tlsConfig.VerifyPeerCertificate,
		...
		MinVersion:             rt.ja.builder.cli.tlsConfig.MinVersion,
		MaxVersion:             rt.ja.builder.cli.tlsConfig.MaxVersion,
		Time:                   rt.ja.builder.cli.tlsConfig.Time,
		Rand:                   rt.ja.builder.cli.tlsConfig.Rand,
	}
	if name := rt.ja.builder.cli.tlsConfig.ServerName; name != "" {
		config.ServerName = name
	}
	if native := rt.ja.builder.cli.fathomry.control.JAConfig; native != nil {
		config = native.Clone()
	...
	if rt.ja.builder.cli.fathomry.control.JAConfig == nil && supportsResumption(spec) && rt.clientSessionCache != nil {
	...
	if err = uconn.HandshakeContext(ctx); err != nil {
		return nil, fmt.Errorf("uTLS.HandshakeContext() error: %w", err)
	}
