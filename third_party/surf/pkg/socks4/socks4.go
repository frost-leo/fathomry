...
func (s socks4) DialContext(ctx context.Context, network, addr string) (c net.Conn, err error) {
	...
	stop := context.AfterFunc(ctx, func() { _ = acquired.Close(); close(done) })
	defer func() {
		if !stop() {
			<-done
		}
	...
	identity := Ident
	if s.url.User != nil {
		identity = s.url.User.Username()
	}
	req, err := request{Host: host, Port: port, IP: ip, Is4a: s.isSocks4a(), Ident: identity}.Bytes()
	if err != nil {
