...
type proxyDialer struct {
	...
	Dialer            net.Dialer
	DialSocket        func(context.Context, string, string) (net.Conn, error)
	DisableReuse      bool
...
func NewDialer(proxy string) (*proxyDialer, error) {
	...
	if parsed.User != nil && (parsed.Scheme == schemeHTTP || parsed.Scheme == schemeHTTPS) {
...
func (c *proxyDialer) connectHTTP1(req *http.Request, conn net.Conn) (net.Conn, error) {
	...
	maximum := c.MaxHeaderBytes
	if maximum <= 0 {
		maximum = 1 << 20
	}
	limited := &io.LimitedReader{R: conn, N: maximum + 1}
	...
	if maximum+1-limited.N-int64(reader.Buffered()) > maximum {
		_ = conn.Close()
		return nil, errors.New("surf: proxy response headers exceed limit")
	}
...
func (c *proxyDialer) connectHTTP2(
	...
	connecting := req.Context()
	if c.ConnectionContext != nil {
		req = req.WithContext(c.ConnectionContext)
	}
	done := make(chan struct{})
	stop := context.AfterFunc(connecting, func() { _ = conn.Close(); close(done) })
	defer func() {
		if !stop() {
			<-done
		}
	...
	if err != nil {
		...
		return nil, errors.Join(err, connecting.Err(), context.Cause(connecting))
	}
	var size int64
	...
	if c.MaxHeaderBytes > 0 && size > c.MaxHeaderBytes {
		return nil, errors.Join(errors.New("surf: proxy response headers exceed limit"), pw.Close(), pr.Close(), resp.Body.Close(), conn.Close())
	}
...
func (c *proxyDialer) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if c.Dialer.Resolver != nil {
		...
		if net.ParseIP(host) == nil {
			...
			for _, ip := range ips {
				selected = ip.IP
				if c.ProxyURL.Scheme == socks4 {
					selected = selected.To4()
				}
				if selected != nil {
					break
				}
			...
			if selected == nil {
				return nil, &net.DNSError{Err: "configured resolver returned no usable address", Name: host}
			}
			address = net.JoinHostPort(selected.String(), port)
		}
	}

	if strings.HasPrefix(c.ProxyURL.Scheme, "socks") {
		forward := proxy.Dialer(&dialerProxy{dialer: &c.Dialer, dialContext: c.DialSocket})
	...
	if !c.DisableReuse && c.h2Conn != nil && c.conn != nil && c.h2Conn.CanTakeNewRequest() {
		rc := c.conn
		cc := c.h2Conn
		c.h2Mu.Unlock()
		unlocked = true
...
func (c *proxyDialer) initProxyConn(ctx context.Context, network string) (net.Conn, string, error) {
	...
	case schemeHTTP:
		rawConn, err = c.dialSocket(ctx, network, c.ProxyURL.Host)
		if err != nil {
			return nil, "", err
		}
	case schemeHTTPS:
		if c.DialTLSContext != nil {
			...
			tcpConn, err := c.dialSocket(ctx, network, c.ProxyURL.Host)
			if err != nil {
				return nil, "", err
			}
...
func (c *proxyDialer) connect(req *http.Request, conn net.Conn, negotiatedProtocol string) (net.Conn, error) {
	if negotiatedProtocol == http2.NextProtoTLS {
		c.tr2Once.Do(func() {
			...
			if c.MaxHeaderBytes > 0 {
				c.tr2.MaxHeaderListSize = uint32(c.MaxHeaderBytes)
			}
		})
	...
	return c.connectHTTP1(req, conn)
}

func (c *proxyDialer) dialSocket(ctx context.Context, network, address string) (net.Conn, error) {
	if c.DialSocket != nil {
		return c.DialSocket(ctx, network, address)
	}
