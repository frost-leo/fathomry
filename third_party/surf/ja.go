...
func (j *JA) build() *Builder {
	return j.builder.addCliMW(func(c *Client) error {
		...
		if err := fathomryValidateH2(j.builder.http2settings); err != nil {
			return err
		}
...
func (j *JA) getSpec(contexts ...context.Context) g.Result[utls.ClientHelloSpec] {
	var spec *utls.ClientHelloSpec

	if factory := j.builder.cli.fathomry.control.HelloSpecFactory; factory != nil {
		...
		if err != nil {
			return g.Err[utls.ClientHelloSpec](err)
		}
		spec = &value
	} else if !j.id.IsSet() {
