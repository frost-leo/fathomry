...
func (req *Request) Do() (result g.Result[*Response]) {
	// Return early if request has preparation errors
	if req.err != nil {
		return g.Err[*Response](fathomryFailure(req.err, closeFathomryBody(req.request)))
	}
	if req.cli == nil || req.cli.fathomry == nil || req.request == nil {
		return g.Err[*Response](ErrFathomryClosed)
	}
	defer func() {
		if failed := req.cli.fathomry.failure(); failed != nil {
			if result.IsErr() {
				result = g.Err[*Response](errors.Join(result.Err(), failed))
			} else {
				response := result.Ok()
				var cleanup error
				...
				result = g.Err[*Response](fathomryFailure(failed, cleanup))
			}
		}
	}()
	end, err := req.cli.fathomry.enter(req.request.Context())
	if err != nil {
		return g.Err[*Response](fathomryFailure(err, closeFathomryBody(req.request)))
	}
	defer end()
	ctx, cancel := context.WithCancel(req.request.Context())
	ctx = http2.FathomryWithWork(ctx, req.cli.fathomry.enter)
	ctx = http3.FathomryWithWork(ctx, req.cli.fathomry.enter)
	stop := context.AfterFunc(req.cli.fathomry.ctx, cancel)
	owned := *req
	owned.request = req.request.Clone(ctx)
	req = &owned
	handed := false
	defer func() {
		if !handed {
	...
	if req.request.Method != http.MethodHead {
		if req.multipart == nil || req.multipart.retry {
			...
			if req.bodyBytes != nil && req.request.GetBody == nil {
				data := req.bodyBytes
	...
	start := time.Now()
	cli := req.cli.cli
	if configure := req.cli.fathomry.control.RequestClient; configure != nil {
		configured, err := configure(req.request.Context(), *cli)
		if err != nil {
			return g.Err[*Response](fathomryFailure(err, closeFathomryBody(req.request)))
		}
		cli = &configured
...
retry:
	...
	if req.request.Method == http.MethodHead || resp.StatusCode == 204 || resp.StatusCode == 304 || resp.Uncompressed {
		expected = -1
	}
	resp.Body = &fathomryFraming{ReadCloser: resp.Body, expected: expected, maximum: req.cli.fathomry.control.MaxResponseBytes}
	...
	if builder != nil && builder.retryMax != 0 && attempts < builder.retryMax && !builder.retryCodes.IsEmpty() &&
		...
		if readErr != nil || closeErr != nil {
			return g.Err[*Response](fathomryFailure(readErr, closeErr))
		}
		attempts++
	...
	if err := req.cli.applyRespMW(response); err != nil {
		...
		if response.Body != nil {
			...
			cleanup = resp.Body.Close()
		}
		return g.Err[*Response](fathomryFailure(err, cleanup))
	}
	if response.Body != nil {
		response.Body.Reader = &fathomryContextBody{ReadCloser: response.Body.Reader, cancel: cancel, stop: stop}
		handed = true
	} else if resp.Body != nil {
		if err := resp.Body.Close(); err != nil {
			return g.Err[*Response](fathomryFailure(nil, err))
		}
	}
