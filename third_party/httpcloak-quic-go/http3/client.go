...
type ClientConn struct {
	senderMu      sync.Mutex
	senders       map[*requestSender]struct{}
	sendersClosed bool
	conn          *quic.Conn
	rawConn       *rawConn
...
func (c *ClientConn) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil && req.Context().Err() != nil {
		...
		err = errors.Join(err, req.Context().Err(), context.Cause(req.Context()))
	}
	return rsp, err
}
...
func (c *ClientConn) sendRequestBody(str *RequestStream, body io.ReadCloser, contentLength int64) (resultErr error) {
	...
	if n > contentLength {
		str.CancelWrite(quic.StreamErrorCode(ErrCodeRequestCanceled))
		return errors.Join(fmt.Errorf("http: ContentLength=%d with Body length %d", contentLength, n), err)
	}
	return err
}

func (c *ClientConn) doRequest(req *http.Request, str *RequestStream) (response *http.Response, resultErr error) {
	var sender *requestSender
	if req.Body != nil {
		var err error
		sender, err = c.startSender(req.Body)
		if err != nil {
			return nil, errors.Join(err, req.Body.Close())
		}
		sender.abortWrite = func() { str.CancelWrite(quic.StreamErrorCode(ErrCodeRequestCanceled)) }
		req = req.Clone(req.Context())
		req.Body = sender
		defer func() {
			if response != nil {
				response.Body = &senderResponse{ReadCloser: response.Body, sender: sender}
			} else {
				select {
				case <-sender.done:
				default:
				...
				<-sender.done
				resultErr = errors.Join(resultErr, closeErr, sender.sendErr)
			}
		}()
	}
	...
	if !sendingReqFailed {
		if req.Body == nil {
			...
			go func() {
				var sendErr error
				defer func() { sender.finish(errors.Join(sendErr, str.Close())) }()
	...
	for {
		...
		if is1xxNonTerminal {
			...
			if err := traceGot1xxResponse(trace, resCode, textproto.MIMEHeader(res.Header)); err != nil {
				str.CancelRead(quic.StreamErrorCode(ErrCodeRequestCanceled))
				str.CancelWrite(quic.StreamErrorCode(ErrCodeRequestCanceled))
				return nil, err
			}
