...
func (r *roundTripperWithCount) Close() error {
	...
	if client, ok := r.clientConn.(interface{ FathomryCloseSenders() error }); ok {
...
func (t *Transport) doRoundTripOpt(req *http.Request, opt RoundTripOpt, isRetried bool) (*http.Response, error) {
	...
	if cl.dialErr != nil {
		return nil, errors.Join(cl.dialErr, t.removeClient(hostname, cl))
	}
	...
	if err != nil {
		...
		if cleanup := t.removeClient(hostname, cl); cleanup != nil {
			return rsp, errors.Join(err, cleanup)
		}
...
func canRetryRequest(err error, req *http.Request) (*http.Request, error) {
	...
	if req.GetBody != nil {
		newBody, retryErr := req.GetBody()
		if retryErr != nil {
			var cleanup error
			if newBody != nil {
				cleanup = newBody.Close()
			}
			return nil, errors.Join(err, retryErr, cleanup)
		}
		if newBody == nil {
			return nil, errors.Join(err, errors.New("http3: replay factory returned no body"))
		}
		reqCopy := *req
...
func (t *Transport) removeClient(hostname string, expected *roundTripperWithCount) error {
	t.mutex.Lock()
...
func (t *Transport) Close() error {
	...
	for _, cl := range t.clients {
		causes = append(causes, cl.Close())
	}
	t.clients = nil
	if t.transport != nil {
		causes = append(causes, t.transport.Close(), t.transport.Conn.Close())
		t.transport = nil
	}
	t.closed = true
