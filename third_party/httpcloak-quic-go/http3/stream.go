...
func (s *RequestStream) ReadResponse() (*http.Response, error) {
	...
	wireLength := res.ContentLength
	if s.isHead || res.StatusCode == http.StatusNotModified || res.StatusCode == http.StatusNoContent || res.StatusCode < 200 {
		wireLength = 0
	}
	respBody := newResponseBody(s.str, wireLength, s.reqDone)
	...
	if s.requestedGzip && !s.isHead && !isInformational && !isNoContent && res.StatusCode != http.StatusNotModified && res.Header.Get("Content-Encoding") == "gzip" {
