...
func limited(t *testing.T, limits resource.Limits, release resource.ReleaseFunc) (*resource.Assembly, resource.Selection[reader], *resource.Access) {
	t.Helper()
	selected := resource.WithLimits(selection(t, "controlled", release), limits)
...
func limits() resource.Limits {
	return resource.Limits{Active: 1, Queued: 1, Bytes: 8, QueuedBytes: 8, MaxLeases: 4}
}

func TestAdmissionRejectsInvalidPolicyBeforeConstruction(t *testing.T) {
	for _, policy := range []resource.Limits{
		...
		spec := resource.WithLimits(resource.Select(prepared(t, "invalid", "example", ""), func(context.Context, settings) (resource.Resource[reader], error) {
			calls.Add(1)
			return resource.Resource[reader]{}, nil
		}), policy)
		if assembly, err := resource.Assemble(context.Background(), context.Background(), "invalid", spec); assembly != nil || !errors.Is(err, resource.ErrSelection) || calls.Load() != 0 {
			t.Fatal("invalid policy reached construction")
		}
	}
	owner, selected, _ := limited(t, limits(), complete)
	override := resource.WithLimits(resource.Borrow("alias", owner, selected), limits())
	if assembly, err := resource.Assemble(context.Background(), context.Background(), "override", override); assembly != nil || !errors.Is(err, resource.ErrSelection) {
	...
	if _, err := resource.AccessFor(assembly, unlimited); !errors.Is(err, resource.ErrSelection) {
...
func TestAdmissionFIFOBytesAndCancellation(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		synctest.Wait()
		if usage := assembly.Snapshot().Sources[0].Usage; usage != (resource.Usage{Active: 1, ActiveBytes: 8, Queued: 1, QueuedBytes: 5}) {
			t.Fatalf("wrong dimensions: %+v", usage)
		}
		if lease, err := access.Acquire(context.Background(), 4); lease != nil || !errors.Is(err, resource.ErrCapacity) {
...
func TestCallBorrowingPreservesScopeShutdownAndDependencies(t *testing.T) {
	...
	owner, selected, access := limited(t, limits(), func(ctx context.Context) resource.ReleaseResult {
		closed.Add(1)
		return complete(ctx)
	})
	...
	if err := owner.Close(context.Background()); !errors.Is(err, resource.ErrIncomplete) {
		t.Fatal("owner ignored scope borrower")
	}
	if lease, err := access.Acquire(context.Background(), 0); lease != nil || !errors.Is(err, resource.ErrSelection) {
	...
	dependency := selection(t, "dependency", func(ctx context.Context) resource.ReleaseResult {
		dependencyClosed.Add(1)
		return complete(ctx)
	})
	consumer := resource.WithLimits(selection(t, "consumer", complete), limits())
	assembly := assemble(t, "ordered", dependency, consumer)
	use, err := resource.AccessFor(assembly, consumer)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := assembly.Close(context.Background()); !errors.Is(err, resource.ErrIncomplete) || dependencyClosed.Load() != 0 {
...
func TestBorrowTreeChargesOneUseAndBoundsRetainedAncestors(t *testing.T) {
	...
	root.Release()
	child.Release()
	if _, err := grandchild.Retain(); !errors.Is(err, resource.ErrCapacity) {
		t.Fatal("released ancestors escaped memory bound")
	}
	if _, err := child.Retain(); !errors.Is(err, resource.ErrAdmission) {
	...
	if receiver, err := resource.Assemble(context.Background(), context.Background(), "receiver", resource.Delegate("take", assembly, selected)); receiver != nil || !errors.Is(err, resource.ErrSelection) {
		t.Fatal("active operation delegated")
	}
	grandchild.Release()
	grandchild.Release()
	for _, lease := range []*resource.Lease{root, child, grandchild} {
		select {
		case <-lease.Done():
		default:
...
func TestConcurrentLeaseReleaseAndCloseAreIdempotent(t *testing.T) {
	...
	assembly, _, access := limited(t, policy, func(ctx context.Context) resource.ReleaseResult {
		releases.Add(1)
		return complete(ctx)
	})
...
func TestRuntimeAccessAndLeaseCannotBeSerialized(t *testing.T) {
	...
	for _, value := range []any{new(resource.Access), new(resource.Lease)} {
...
func TestCanceledCloseWithActiveCallsKeepsCancellationCause(t *testing.T) {
	...
	for range 100 {
		...
		if !errors.Is(err, resource.ErrIncomplete) || !errors.Is(err, context.Canceled) || !errors.Is(err, cause) {
...
func TestAliasesCannotMultiplyAdmission(t *testing.T) {
	...
	borrowed := resource.Borrow("second", owner, selected)
	borrower := assemble(t, "borrower", borrowed)
	defer borrower.Close(context.Background())
	second, err := resource.AccessFor(borrower, borrowed)
	if err != nil {
		t.Fatal(err)
	}
	...
	if lease, err := second.Acquire(context.Background(), 0); lease != nil || !errors.Is(err, resource.ErrCapacity) {
...
func TestActiveBytesAreIndependentOfAvailableCallSlots(t *testing.T) {
	...
	if lease, err := access.Acquire(context.Background(), 2); lease != nil || !errors.Is(err, resource.ErrCapacity) {
