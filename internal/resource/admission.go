...
func AccessFor[C any](assembly *Assembly, selected Selection[C]) (*Access, error) {
	fail := func() (*Access, error) {
		return nil, ErrSelection.New(fault.Context{Operation: "access"})
	}
	...
	for _, entry := range assembly.entries {
		...
		if !validLimits(resource.admission.limits) || resource.released ||
...
func (access *Access) Info() Info {
	...
	resource.mu.Lock()
	defer resource.mu.Unlock()
	return Info{Scope: resource.info.Scope, Configuration: resource.info.Configuration.Clone()}
}
...
func (access *Access) Acquire(ctx context.Context, bytes int64) (*Lease, error) {
	if access == nil || access.assembly == nil || access.entry == nil || ctx == nil || bytes < 0 {
		return nil, ErrAdmission.New(fault.Context{Operation: "acquire"})
	}
	assembly, entry := access.assembly, access.entry
	resource := entry.record
	location := location(resource.info.Scope, resource.info.Configuration, "acquire")
	...
	for {
		...
		case !validLimits(policy.limits) || bytes > policy.limits.Bytes:
...
func (lease *Lease) Retain() (*Lease, error) {
	if lease == nil || lease.node == nil {
		return nil, ErrAdmission.New(fault.Context{Operation: "retain"})
	}
	node := lease.node
	family := node.family
	family.mu.Lock()
	...
	if family.nodes >= family.limit {
		return nil, ErrCapacity.New(fault.Context{Operation: "retain"})
	}
	family.nodes++
	node.references++
