...
func TestInitializationCancellationRetainsCause(t *testing.T) {
	for _, phase := range []string{"before-construction", "after-construction", "after-readiness"} {
		t.Run(phase, func(t *testing.T) {
			...
			selected := resource.Select(prepared(t, "review", "example.reader", ""), func(context.Context, settings) (resource.Resource[reader], error) {
				if phase == "after-construction" {
					cancel(cause)
				}
				return resource.Resource[reader]{
			...
			assembly, err := resource.Assemble(ctx, context.Background(), "review", selected)
		...
		for _, phase := range []string{"before-construction", "after-construction", "after-readiness"} {
			t.Run(kind+"/"+phase, func(t *testing.T) {
				synctest.Test(t, func(t *testing.T) {
					...
					defer cancelCleanup()
					var order []string
					selected := resource.Select(prepared(t, "one", "example.reader", ""), func(input context.Context, _ settings) (resource.Resource[reader], error) {
						...
						return resource.Resource[reader]{Acquired: true, Capability: readerFacade{},
							Check: func(input context.Context) error {
								...
								return nil
							}, Release: func(input context.Context) resource.ReleaseResult {
					...
					assembly, err := resource.Assemble(ctx, cleanupCtx, "cancellation", selected)
					if assembly == nil || !errors.Is(err, resource.ErrAssembly) || ctx.Err() != identity || context.Cause(ctx) != expectedCause {
					...
					for _, result := range []error{err, report.Primary} {
						if !errors.Is(result, identity) || !errors.Is(result, expectedCause) || !errors.Is(result, resource.ErrInitialization) {
					...
					var primary *fault.Error
					if !errors.As(report.Primary, &primary) || primary.Diagnostic().Context.Operation != operation ||
						report.Ready || !reflect.DeepEqual(order, expectedOrder) {
						t.Error("cancellation phase, readiness or callback ordering changed")
					}
					if _, _, bindErr := resource.Bind(assembly, selected); !errors.Is(bindErr, resource.ErrSelection) {
	...
	for _, path := range []string{"framework", "constructor-error", "readiness-error"} {
		for _, cleanup := range []string{"continue", "no-continuation", "released"} {
			t.Run(path+"/"+cleanup, func(t *testing.T) {
				...
				callbackIdentity := fault.Kind("example.reader.initialization")
				callbackErr := callbackIdentity.New(fault.Context{}, native)
				cleanupCause := errors.New("cleanup-canary")
				cleanupCtx, cancelCleanup := context.WithCancel(context.Background())
				defer cancelCleanup()
				var order []string
				dependency := selection(t, "dependency", func(input context.Context) resource.ReleaseResult {
					if input != cleanupCtx || input.Err() != nil {
				...
				consumer := resource.Select(prepared(t, "consumer", "example.reader", ""), func(context.Context, settings) (resource.Resource[reader], error) {
					resource := resource.Resource[reader]{Acquired: true, Capability: readerFacade{},
						Release: func(input context.Context) resource.ReleaseResult {
							if input != cleanupCtx || input.Err() != nil {
								t.Error("consumer cleanup inherited cancellation or lost its caller context")
							}
							order = append(order, "consumer")
							result := resource.ReleaseResult{Err: cleanupCause}
							if cleanup == "released" {
								result.Quiescent, result.Released = true, true
							} else if cleanup == "continue" {
								result.Continue = func(input context.Context) resource.ReleaseResult {
									if input != cleanupCtx {
				...
				assembly, err := resource.Assemble(ctx, cleanupCtx, "failed", dependency, consumer)
				if assembly == nil || !errors.Is(err, resource.ErrAssembly) || !errors.Is(err, cleanupCause) {
				...
				if errors.Is(err, resource.ErrIncomplete) != pending || report.Sources[0].Pending != pending ||
				...
				for range 2 {
					...
					if !errors.Is(closeErr, cleanupCause) || errors.Is(closeErr, resource.ErrIncomplete) != (cleanup == "no-continuation") ||
...
func TestInitializationCancellationWithExpiredCleanupRetainsResponsibility(t *testing.T) {
	...
	selected := resource.Select(prepared(t, "one", "example.reader", ""), func(context.Context, settings) (resource.Resource[reader], error) {
		cancel(initCause)
		return resource.Resource[reader]{Acquired: true, Capability: readerFacade{}, Release: func(input context.Context) resource.ReleaseResult {
			if input.Err() != nil {
	...
	assembly, err := resource.Assemble(ctx, cleanupCtx, "expired-cleanup", selected)
	if assembly == nil {
		t.Fatal("acquired responsibility lost")
	}
	for _, cause := range []error{context.Canceled, initCause, context.DeadlineExceeded, cleanupCause, resource.ErrIncomplete} {
